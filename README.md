...
| packages | `/packages/v2/active`, `/packages/v2/inactive`, `/packages/v2/onhold`, `/packages/v2/types`, `/packages/v2/adjust/reasons`, `/packages/v2/{id}`, `/packages/v2/{label}`, `/packages/v2/adjustments`, `/packages/v2/transferred`, `/packages/v2/intransit`, `/packages/v2/labsamples`, `/packages/v2/{id}/source/harvests` | complete | All documented endpoints implemented and tested. `/packages/v2/adjustments` is the actual "history" endpoint that closed [issue #13](https://github.com/yerba-buena/metrc-ny-client/issues/13). |
...
| sales | `/sales/v2/receipts/active`, `/sales/v2/receipts/{id}`, `/sales/v2/receipts/inactive`, `/sales/v2/receipts/external/{externalNumber}`, `/sales/v2/customertypes`, `/sales/v2/patientregistration/locations`, `/sales/v2/deliveries/returnreasons`, `/sales/v2/deliveries/active`, `/sales/v2/deliveries/inactive`, `/sales/v2/deliveries/{id}`, `/sales/v2/deliveries/retailer/active`, `/sales/v2/deliveries/retailer/inactive`, `/sales/v2/deliveries/retailer/{id}`, `/sales/v2/counties`, `/sales/v2/paymenttypes` | complete | All 15 documented endpoints implemented (Phase 7). 4 endpoints (`/counties`, `/paymenttypes`, `/deliveries/{id}`, `/deliveries/retailer/{id}`) return 401 on the current license — URL patterns implemented with permissive schemas pending license access. `/sales/v2/transactions` removed — URL does not exist in METRC NY v2. See [`src/coverage.ts`](src/coverage.ts). |
